...
```markdown
## R1
CUANDO el usuario envía POST /api/v1/pipelines con datos válidos, el sistema DEBE
crear un pipeline y devolver 201 con el payload del recurso creado.

## R2
SI el payload incluye un servidor de tipo "local" en los targets ENTONCES el sistema DEBE
devolver 422 con un mensaje de error indicando que servidores locales no están permitidos en pipelines.

## R3
MIENTRAS un pipeline tenga ejecuciones activas, el sistema NO DEBE permitir
la actualización ni eliminación del pipeline.
```
...
```markdown
- [ ] T1 — Crear `PipelineTarget` value object en `app/v1/pipelines/domain/value_objects/`. Cubre: R2.
- [ ] T2 — Crear `CreatePipeline` command en `app/v1/pipelines/application/commands/`. Cubre: R1, R2.
- [ ] T3 — Añadir `test_create_pipeline_valid` en `tests/v1/pipelines/test_use_cases/`. Cubre: R1.
- [ ] T4 — Añadir `test_create_pipeline_local_server_rejected` en `tests/v1/pipelines/test_use_cases/`. Cubre: R2.
- [ ] T5 — Añadir `test_update_pipeline_with_active_executions_rejected` en `tests/v1/pipelines/test_use_cases/`. Cubre: R3.
```
...
SDD define **roles**, no personas. En este proyecto los roles se materializan
como subagentes de IA definidos en `.opencode/agents/`:

| Rol | Subagente | Responsabilidad |
|-----|-----------|----------------|
| `spec_author` | `.opencode/agents/spec_author.md` | Redacta `requirements.md`, `design.md`, `tasks.md` |
| `implementer` | `.opencode/agents/implementer.md` | Escribe código y tests siguiendo `tasks.md` |
| `reviewer` | `.opencode/agents/reviewer.md` | Valida trazabilidad `R<n>` ↔ test y tasks completas |
| `leader` | `.opencode/agents/leader.md` | Orquesta: detecta features, lanza subagentes, pide aprobación humana |

El `leader` es el rol principal de OpenCode (ver `FIRST_AGENT.md`). No edita
código directamente — lanza el subagente apropiado para cada fase.
