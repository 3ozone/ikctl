...
### Caso E — subagente devuelve vacío o falla

Si el subagente `implementer` o `reviewer` responde con resultado vacío,
error o sin referencia a un archivo en `progress/`:

1. **No declares la tarea como done.** El trabajo no está verificado.
2. **Asume el rol tú mismo**, siguiendo el protocolo de `implementer.md`
   al pie de la letra — incluyendo los pasos de `progress/`:
   - Anota en `progress/current.md` la feature en curso y el plan ANTES de empezar.
   - Implementa las tasks de `specs/<name>/tasks.md` en orden, marcando `[x]` al completar cada una.
   - Crea `progress/impl_<name>.md` con el mapa `R<n> → test` al terminar.
   - Si la feature toca `routes.py` / `schemas.py`, ejecuta `python scripts/export_openapi.py`.
3. Una vez el trabajo esté hecho y los tests verdes, vacía `progress/current.md`
   dejando solo la plantilla y mueve el resumen a `progress/history.md`.

> **Regla:** si asumes el rol de implementer, tienes permitido editar `app/`
> y `tests/` solo en este escenario de fallback. Documéntalo en
> `progress/current.md` con la nota "leader actuando como implementer (fallback)".
