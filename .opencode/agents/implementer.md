...
3. **Anota en `progress/current.md` ANTES de escribir ningún código**:
   ```markdown
   ## Feature en curso
   <id> — <name>

   ## Plan
   Tasks T1..Tn de specs/<name>/tasks.md

   ## Notas
   (vacío al inicio)
   ```
   Si `progress/current.md` no se actualiza, el leader no puede saber qué
   está en curso. Este paso es **obligatorio**, no opcional.
...
5. **Actualiza el contrato OpenAPI** si la feature añade o modifica endpoints
   (cualquier cambio en `routes.py` o `schemas.py`):
   ```bash
   python scripts/export_openapi.py
   ```
   Si no hay cambios en la capa de presentación, este paso se omite.
...
7. **Trazabilidad**: confirma que cada `R<n>` está cubierto por al menos
   ...
   (mapa `R<n> → test`). Este archivo es **obligatorio** — sin él el
   reviewer no puede aprobar.
8. **No marques `done` tú mismo.** Espera al reviewer.
9. Si el reviewer aprueba (te lo dirá el leader en una segunda invocación):
   cambias estado a `done`, vacías `progress/current.md` dejando solo la
   plantilla, y mueves el resumen a `progress/history.md`.
...
- ❌ No escribas código sin haber actualizado primero `progress/current.md`.
- ❌ No declares trabajo hecho sin `progress/impl_<name>.md` creado.
