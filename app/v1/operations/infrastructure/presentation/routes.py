...
Endpoints:
  POST   /api/v1/operations              — T-28: lanzar operación (single o batch)
...
# ---------------------------------------------------------------------------
# T-28: POST /api/v1/operations — lanzar operación (single o batch)
# ---------------------------------------------------------------------------


@router.post(
    "/api/v1/operations",
    response_model=Union[OperationResponse, BatchOperationResponse],
    status_code=status.HTTP_201_CREATED,
)
async def launch_operation(
    body: LaunchOperationRequest,
    user_id: Annotated[str, Depends(get_current_user_id)],
    single_uc: Annotated[LaunchOperation, Depends(get_launch_operation_uc)],
    batch_uc: Annotated[LaunchBatchOperation, Depends(get_launch_batch_operation_uc)],
) -> Union[OperationResponse, BatchOperationResponse]:
    """Lanza una operación asíncrona ejecutando un kit en un servidor o grupo.

    Si se proporciona `server_id` lanza una operación individual y devuelve
    `OperationResponse`. Si se proporciona `group_id` lanza una operación por
    cada servidor del grupo y devuelve `BatchOperationResponse`.

    Returns:
        201 OperationResponse (single) o BatchOperationResponse (batch).

    Raises:
        400: Si se proporcionan ambos o ninguno de server_id/group_id.
        404: Si el grupo no existe.
        422: Si el servidor está inactivo o el kit no está sincronizado.
    """
    if body.server_id is not None:
        correlation_id = str(uuid4())
        result: OperationResult = await single_uc.execute(
            user_id=user_id,
    ...
    # group_id is not None (validated by schema)
    batch_result: BatchOperationResult = await batch_uc.execute(
        user_id=user_id,
        group_id=body.group_id,  # type: ignore[arg-type]
        kit_id=body.kit_id,
        debug_level=body.debug_level,
        values=body.values,
        sudo=body.sudo,
    ...
    logger.info(
        "batch_operation_launched",
        user_id=user_id,
        group_id=body.group_id,
        kit_id=body.kit_id,
        count=len(batch_result.operations),
    )
    return BatchOperationResponse(
        operations=[_to_operation_response(op) for op in batch_result.operations]
    )
