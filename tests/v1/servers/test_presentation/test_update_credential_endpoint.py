...
class FakeUpdateCredentialUnauthorized:
    ...
    async def execute(self, **kwargs) -> CredentialResult:
        ...
        raise UnauthorizedOperationError("No tienes permiso para actualizar esta credencial.")
...
def test_update_credential_returns_200(client_ok: TestClient) -> None:
    ...
    resp = client_ok.put(f"/api/v1/credentials/{_CRED_ID}", json=payload, headers=_auth_headers())
...
def test_update_credential_not_found_returns_404(client_not_found: TestClient) -> None:
    ...
    resp = client_not_found.put(f"/api/v1/credentials/{_CRED_ID}", json=payload, headers=_auth_headers())
...
def test_update_credential_unauthorized_returns_403(client_unauthorized: TestClient) -> None:
    ...
    payload = {"name": "updated-key"}
    resp = client_unauthorized.put(f"/api/v1/credentials/{_CRED_ID}", json=payload, headers=_auth_headers())
